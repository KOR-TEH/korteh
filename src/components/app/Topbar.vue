...
<!-- eslint-disable -->
    ...
    <Menu v-if="topbarmenu_open" id="topbar-menu" ref="menu" :model="menuItems" :popup="true" />
	...
	<!--Menu id="overlay_menu" ref="menu" :model="menuItems" :popup="true" /-->
        ...
        <ul class="layout-topbar-menu lg:flex origin-top">
            <li>
               <span style="font-size: 1.2rem; margin-left:1rem">Кортех</span>
            </li>
        </ul>			
...
onMounted (
  async () => {
    ...
    //const route_ = router //router.currentRoute.value
	//console.log('router-app-bar',router.currentRoute.value)
	//console.log('route-app-bar',route)
	//if(router.currentRoute.value == 'projects') isPageProjects.value = true
  }
)
...
const startAddProject = (event: PointerEvent) => {
  ...
  //router.params.emitProjectAdd() 
  console.log('add-project-emit',router)
  console.log('emit',menustate.usedemit)
...
const toggle = (event: PointerEvent) => {
  //menu.value.toggle(event)
    //emit('user-menu-toggle', event)
	topbarmenu_open.value = !topbarmenu_open.value
	console.log('user-menu-toggle',event,menu)
}
/*
