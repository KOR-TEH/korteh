          ...
          <Dialog header="Добавить субтитры" v-model:visible="showAddSubs" :breakpoints="{'960px': '75vw', '640px': '90vw'}" :style="{width: '50vw'}" :modal="true">
            ...
            <template #footer>
                ...
                <Dialog header="Новое сканирование" @addProject="emitProjectAdd" v-model:visible="showAddVideo" :breakpoints="{ '960px': '75vw' }" :style="{ width: '30vw' }" :modal="true">
					...
					<div class="formgrid grid">
						<div class="field col">
								<label for="frequency">Загрузка видеофайла</label>
                                <FileUpload name="file[]" :customUpload="true" @uploader="importUploader" mode="basic" ref='file' accept="" :maxFileSize="50000000000" label="Видео" chooseLabel="Видео" class="mr-2" />					
							</div>
							<div class="field col">
								<label for="frequency">Загрузка субтитров</label>
			                    <FileUpload name="subtitles[]" :customUpload="true" @uploader="importSubsUploader" mode="basic" ref='file' accept="" :maxFileSize="50000000000" label="Субтитры" chooseLabel="Субтитры" class="mr-2" />							
							</div>
					</div>
...
const watchVideoStatus = (colId, rowId, id) => {
  ...
  interval[id] = window.setInterval(() => {  
	...
	projects.value[colId].video[rowId].status = (raw)? raw.complete :''
	//raw.status === 'Ready'
	if(raw?.status === 'Выполнено') window.clearInterval(interval[id])
	if(raw?.status === 'Выполнено') projects.value[colId].video[rowId].status = 'Выполнено'	
  },1000)
}
